...
The **AI Models** layer (under Platforms & Software) stands in for the model makers. *US Frontier Labs* holds the US frontier labs that trade on their own: Alphabet (Google DeepMind, Gemini) and Meta (Llama, Meta Superintelligence Labs). OpenAI and Anthropic are private, so they are proxied by their largest listed backers: *OpenAI Proxies* holds Microsoft (the largest shareholder), Oracle (Stargate compute contracts) and SoftBank (Stargate and a large equity stake); *Anthropic Proxies* holds Alphabet and Amazon (both shareholders and its compute suppliers). *China Model Labs* holds the model developers that do trade: Zhipu and MiniMax (Hong Kong IPOs, January 2026), SenseTime and iFlytek.
