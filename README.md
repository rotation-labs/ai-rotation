...
The **AI Models** layer (under Platforms & Software) stands in for the model makers. *US Frontier Labs* holds the one US frontier lab that trades on its own: Alphabet (Google DeepMind, Gemini). OpenAI and Anthropic are private, so they are proxied by their largest listed backers: *OpenAI Proxies* holds Microsoft (the largest shareholder), Oracle (Stargate compute contracts) and SoftBank (Stargate and a large equity stake); *Anthropic Proxies* holds Alphabet and Amazon (both shareholders and its compute suppliers). These move far more on their other businesses than on the labs. *China Model Labs* holds the model developers that do trade: Zhipu and MiniMax (Hong Kong IPOs, January 2026), SenseTime and iFlytek.
