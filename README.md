...
The **AI Models** layer (under Platforms & Software) stands in for the model makers. *US Frontier Labs* holds the US frontier labs that trade on their own: Alphabet (Google DeepMind, Gemini) and Meta (Llama, Meta Superintelligence Labs). OpenAI, Anthropic and xAI are private, so each is proxied by its largest listed backers rather than listed directly: *OpenAI Proxies* holds Microsoft (the largest shareholder), Oracle (Stargate compute contracts) and SoftBank (Stargate and a large equity stake); *Anthropic Proxies* holds Alphabet and Amazon (both shareholders and its compute suppliers); *xAI Proxies* holds SpaceX (Elon Musk's other company, and an xAI investor). *China Model Labs* holds the model developers that do trade: Zhipu and MiniMax (Hong Kong IPOs, January 2026), SenseTime and iFlytek.

A proxy basket's members do not all move on the lab equally: OpenAI's fortunes are a much bigger piece of what moves SoftBank's or Oracle's own stock than what moves Microsoft's giant, mostly-unrelated business. *OpenAI Proxies* weights Oracle and SoftBank 3x Microsoft to reflect that (`_weights` in `config/baskets.json`, below); every other basket on the page stays equal-weight, including the rest of AI Models, where the case for a specific ratio is much weaker. The weights are a judgement call, not a fitted or disclosed figure - there is no clean way to measure "exposure" for a private company - so treat the resulting return as a rough tilt, not a precise one, and feel free to change the numbers.

Reading the page: the default Full stack view compares seven tiers against QQQ; opening a layer switches to its industry peer benchmark, and the Benchmark check under the lists shows each call against SPY, QQQ, RSP, SOXX and the peer. Rows show breadth (the share of historically eligible members themselves outperforming) and are sorted by strength, the relative return from about six months to one month ago using the same daily endpoints in Daily and Weekly views. The **Strength** tab prints those exact dates and ranks every dot on it. Strength is descriptive: the checked-in study explores related signals on individual stocks and ETFs, not the seven displayed tier portfolios. The strip above the chart lists recent quadrant changes as descriptive, not as timing signals, and **Compare** plots any two groups against each other using the same dated membership rules as the main chart. View state lives in the URL hash, so any view can be linked and the back button works. `/` focuses the search box. Both axes are scaled by their view-relative RMS distance from 100 and the window is square, so equal on-screen distances mean the same thing either way; the units are not statistical standard deviations and are not comparable between views. **Scale +/−** above the chart (or the `+`, `-` and `0` keys, or a trackpad pinch) zooms up to 8× so small moves show when one name sets the scale; a zoomed window follows the bulk of the current dots, and any dot beyond the edge is pinned to it with an arrow. A badge such as `2/5` marks a quadrant that only holds under the smoothing settings shown — the page re-runs every node under four alternatives. The current, unfinished week is drawn hollow and never earns a NEW badge. **Tickers**, next to Scale, adds each basket's members to its label on the chart (dimmed, after the name) instead of making you hover or tap every dot to see what's inside it; it can make a dense view more crowded, so it defaults off and stays off until you turn it on.
**Movers** lists the individual stocks or S&P sector ETFs in the current view that changed quadrant in the last two bars, grouped neutrally by their destination and labelled with the exact transition. Stocks can be measured against QQQ or their industry peer; sectors use SPY. It is a watchlist, not a signal. Its track-record line re-tests each kind of turn on the full history: weekly bars vs QQQ, the 8-week relative return after the turn minus the average stock that week, with t-statistics computed on 8-week blocks. So far no kind of turn has reliably predicted returns (every |t| < 2), and the universe was chosen in hindsight, which flatters any backtest.
...
`scripts/backtest.py` checks whether readings calculated for individual stocks and ETFs (RS-Ratio, RS-Momentum, their inflection, quadrant moves, and plain relative momentum) predict forward relative returns. It does not construct or test the seven displayed equal-weight tier portfolios, so its results should not be read as validation of the Strength table or a trading strategy. It runs every tested signal over a grid of settings, horizons (1-26 weeks), benchmarks and universes: the AI stocks over the last three years and over ten, about 40 sector and industry ETFs since 2000, and the 11 SPDR sectors. Spreads and ICs carry Newey-West t-stats, judged against a placebo of random signals run through the same code. Run it from **Actions → Backtest → Run workflow**; it also runs on any push that changes it. The report appears in the run summary and as the `backtest` artifact. Locally:
...
- **Edit baskets:** change `config/baskets.json` (tier → layer → subsector → tickers) and commit. Any push to `main` triggers a rebuild. Give every new ticker a display name in `names`; it is shown on hover.
  A subsector (or a layer that lists stocks directly, like Hyperscalers) is normally just an array of tickers, which the page equal-weights. Give it `{"_tickers": [...], "_weights": {"TICKER": n, ...}}` instead when its members' exposure to what the basket tracks genuinely differs (see OpenAI Proxies); a ticker left out of `_weights` defaults to 1. Weighting only changes that basket's own return - a layer or tier above it still equal-weights its distinct stocks, so nothing rolls up. Don't reach for this without a real reason: equal weight is the right default almost everywhere, and a weight you can't defend is worse than none.
